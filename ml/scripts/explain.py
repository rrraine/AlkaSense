...
def get_ai_explanation(image_path: str, asv_score: int,
    ...
    """
    Calls Gemini API with the Grad-CAM overlay image + structured prompt.
    Returns plain-language explanation string.

    Falls back to template string if API is unavailable (offline).
    """
    # Initializes client; automatically picks up GEMINI_API_KEY from environment vars
    client = genai.Client()  
    ...
    try:
        response = client.models.generate_content(
            ...
            contents=[
                # Use types.Part.from_bytes or the dict pattern for inline data
                {
                    "mime_type": "image/png",
                    "data": img_b64
                },
            ...
            # Use GenerateContentConfig to pass max_output_tokens
            config=types.GenerateContentConfig(
                max_output_tokens=300,
                temperature=0.4 # Kept slightly lower for factual scientific explanations
            )
        )
...
def _offline_fallback(asv_score: int, confidence: float) -> str:
    """Template explanation when Gemini API is unreachable."""
    gt_class       = ASV_TO_GT[asv_score]
    gt_range, gt_cook = GT_DESCRIPTIONS[gt_class]
    return (
