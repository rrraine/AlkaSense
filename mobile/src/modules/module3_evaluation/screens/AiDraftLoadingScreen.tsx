...
import { createEvaluationRecord, submitObservationProfile } from '../services/ObservationService';
import { requestAIDraft } from '../services/AIScoringService';
import type { SpreadingPattern, GrainTranslucency, WithinDishUniformity, AnomalyFlag, KOHSolutionAppearance } from '../../../shared/types/evaluation.types';
import type { ObservationInput } from '../services/ObservationService';
...
function toSpreadingPattern(raw: string | null | undefined): SpreadingPattern {
  if (raw === 'Smooth and Continuous') return 'Smooth';
  if (raw === 'Ragged and Fragmented') return 'Ragged';
  if (raw === 'Partial Spreading Only') return 'Partial';
  return 'No Spreading';
}

function toAnomalyFlags(raw: string[] | undefined): AnomalyFlag[] {
  if (!raw || raw.length === 0) return ['None'];
  return raw.map((f) => {
    if (f === 'Longitudinal Cracking') return 'Cracking' as AnomalyFlag;
    if (f === 'Unilateral Spreading')  return 'Unilateral' as AnomalyFlag;
    if (f === 'Floating Grains')       return 'Floating' as AnomalyFlag;
    return 'None' as AnomalyFlag;
  });
}

function buildObservationInput(answers: Record<string, any>): ObservationInput {
  return {
    spreading_pattern:      toSpreadingPattern(answers.spreadingPattern),
    grain_translucency:     (answers.grainTranslucency ?? 'Fully Translucent') as GrainTranslucency,
    within_dish_uniformity: (answers.scoreUniformity ?? 'Uniform') as WithinDishUniformity,
    anomaly_flags:          toAnomalyFlags(answers.anomalyFlags),
    koh_solution_appearance:(answers.kohSolution ?? 'Clear') as KOHSolutionAppearance,
  };
}
...
export default function AiDraftLoadingScreen({ navigation, route }: any) {
  ...
  useEffect(() => {
    ...
    const { imageId = '', imageUri = '', sampleId = '', answers = {} } = params;

    const d0 = STEPS[0].duration;
    const d1 = STEPS[1].duration;
    const d2 = STEPS[2].duration;
    const totalMs = d0 + d1 + d2 + 600;

    t1 = setTimeout(() => setStatuses(['done', 'active', 'pending']), d0);
    t2 = setTimeout(() => setStatuses(['done', 'done', 'active']),     d0 + d1);
    t3 = setTimeout(() => setStatuses(['done', 'done', 'done']),       d0 + d1 + d2);

    // Run inference concurrently with the animation
    const inferenceResultRef: { current: { evaluationId: string; draft: any } | null } = { current: null };

    async function runInference() {
      try {
        console.log('[AiDraftLoading] creating evaluation record — sampleId:', sampleId, 'imageId:', imageId);
        const evalRecord = await createEvaluationRecord(sampleId, imageId);
        const observationInput = buildObservationInput(answers);
        await submitObservationProfile(evalRecord.id, observationInput);
        console.log('[AiDraftLoading] observation saved — evaluationId:', evalRecord.id);

        const draft = await requestAIDraft({
          image_path: imageUri,
          evaluation_id: evalRecord.id,
        });
        console.log('[AiDraftLoading] draft score:', draft.asv_score, 'certainty:', draft.certainty_score);
        inferenceResultRef.current = { evaluationId: evalRecord.id, draft };
      } catch (e: any) {
        console.log('[AiDraftLoading] inference error:', e?.message ?? e);
      }
    }

    const inferencePromise = runInference();

    // Navigate after animation finishes — wait for inference if still running
