...
// Swap this value to preview different UI states while backend is not yet
// integrated. Once the backend is ready, replace this with the actual result
// coming from route.params or an API response.
//
//   'accepted'           → green banner, both pipeline layers pass, "Proceed to Evaluation" CTA
//   'protocol_violation' → orange banner, Layer 1 fails, rejection details + corrective guidance
//   'quality_failure'    → red banner,    Layer 2 fails, rejection details + corrective guidance
// ─────────────────────────────────────────────────────────────────────────────
type ValidationStatus = 'accepted' | 'protocol_violation' | 'quality_failure';
const MOCK_VALIDATION_STATUS: ValidationStatus = 'accepted';
...
interface AcceptedConfig extends BaseConfig {
  status: 'accepted';
}

interface ErrorConfig extends BaseConfig {
  status: 'protocol_violation' | 'quality_failure';
  rejection: { validationLayer: string; failedCondition: string; reason: string };
  corrective: string;
}

type StatusConfig = AcceptedConfig | ErrorConfig;
type PipelineStatus = 'pass' | 'fail' | 'skipped';

// ── Per-status data — swap backend payload in here later ─────────────────────
...
const STATUS_CONFIG: Record<ValidationStatus, StatusConfig> = {
  ...
  protocol_violation: {
    ...
    rejection: {
      ...
      failedCondition: 'Even lighting across sample',
      reason: 'Inadequate lighting condition not confirmed',
    },
  ...
  quality_failure: {
    ...
    rejection: {
      ...
      failedCondition: 'Sharpness and focus',
      reason: 'Image blur level exceeds acceptable threshold',
    },
    corrective: 'Hold the device steady and wait for auto-focus to lock before capturing. Consider using a tripod or stable surface.',
  } satisfies ErrorConfig,
};
...
function StatusBanner({ status }: { status: ValidationStatus }) {
  ...
  const bannerStyle = isAccepted
    ? styles.bannerAccepted
    : isProtocol
    ? styles.bannerProtocol
    : styles.bannerQuality;

  const iconBgStyle = isAccepted
    ? styles.bannerIconAccepted
    : isProtocol
    ? styles.bannerIconProtocol
    : styles.bannerIconQuality;

  const titleStyle = isAccepted
    ? styles.bannerTitleAccepted
    : isProtocol
    ? styles.bannerTitleProtocol
    : styles.bannerTitleQuality;

  const descStyle = isAccepted
    ? styles.bannerDescAccepted
    : isProtocol
    ? styles.bannerDescProtocol
    : styles.bannerDescQuality;
...
function TimingRow({ completedIn }: { completedIn: string }) {
  return (
    <View style={styles.timingRow}>
      ...
      <Text style={styles.timingValue}>{completedIn}</Text>
    </View>
  );
}
...
function PipelineIcon({ pipelineStatus }: { pipelineStatus: PipelineStatus }) {
  if (pipelineStatus === 'pass') {
    return (
      <View style={styles.pipelineIconPass}>
        <Text style={styles.pipelineIconText}>✓</Text>
      </View>
    );
  }
  if (pipelineStatus === 'fail') {
    return (
      <View style={styles.pipelineIconFail}>
        <Text style={styles.pipelineIconText}>✕</Text>
      </View>
    );
  }
  return (
    <View style={styles.pipelineIconSkipped}>
      <Text style={styles.pipelineIconTextSkipped}>–</Text>
    </View>
  );
}

function ValidationPipeline({ layer1, layer2 }: { layer1: PipelineStatus; layer2: PipelineStatus }) {
  return (
  ...
  return (
    <View style={styles.rejectionCard}>
      ...
      <View style={styles.rejectionField}>
        ...
        <Text style={styles.rejectionFieldValue}>{reason}</Text>
      </View>
    </View>
  );
}
...
export default function ValidationResultScreen({ navigation, route }: any) {
  ...
  const config      = STATUS_CONFIG[validationStatus];
  ...
  function handleRecapture() {
    navigation.navigate('ImageCapture', { sampleId, variety, grainCount, session });
  }
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 130 }}>
        ...
        <View style={styles.section}>
          <StatusBanner status={validationStatus} />
        </View>

        {/* TIMING */}
        <TimingRow completedIn={config.completedIn} />

        {/* REJECTION DETAILS — error states only */}
        {errorConfig?.rejection && (
          <View style={styles.section}>
            <RejectionDetails
              validationLayer={errorConfig.rejection.validationLayer}
              failedCondition={errorConfig.rejection.failedCondition}
              reason={errorConfig.rejection.reason}
              isQuality={isQuality}
            />
          </View>
        )}
        ...
        {errorConfig?.corrective && (
          <View style={styles.section}>
            <CorrectiveGuidance message={errorConfig.corrective} />
          </View>
        )}
        ...
        <View style={styles.section}>
          <ValidationPipeline
            layer1={config.pipeline.layer1}
            layer2={config.pipeline.layer2}
          />
        </View>
