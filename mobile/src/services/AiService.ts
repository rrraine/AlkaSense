...
function normalizePredictedGTClass(
  ...
  if (
    value === 'High GT' ||
    value === 'Intermediate GT' ||
    value === 'Low GT'
  ) {
    return value;
  }

  if (value.startsWith('High GT')) {
    return 'High GT';
  }

  if (value.startsWith('Intermediate GT')) {
    return 'Intermediate GT';
  }

  if (value.startsWith('Low GT')) {
    return 'Low GT';
  }
...
export async function requestAIDraft({
  ...
  try {
    ...
    if (observations.grainTranslucency) {
      formData.append(
        'grain_translucency',
        observations.grainTranslucency
      );
    }
    ...
    if (observations.anomalyFlags?.length) {
      formData.append(
        'anomaly_flags',
        observations.anomalyFlags.join(',')
      );
    }
    ...
    return {
      ...result,
      predicted_gt_class:
        normalizePredictedGTClass(
          result.predicted_gt_class
        ),
    };
    ...
    console.warn(
      'requestAIDraft failed, using mock:',
      err
    );
...
export async function requestAIExplainability({
  ...
  try {
    ...
    console.warn(
      'requestAIExplainability failed:',
      err
    );
      ...
      explanation_bullets: [
        {
          ...
          text: 'Unable to load analysis. Check your connection and try again.',
        },
        {
          label: 'Center translucency',
          text: 'Analysis unavailable offline.',
        },
        {
          label: 'Overall morphology',
          text: 'Connect to the backend server to generate AI explanations.',
        },
      ],
    };
  }
...
function mapASVToGTClass(
  score: number
): 'Low GT' | 'Intermediate GT' | 'High GT' {
...
export async function createDraftEvaluation(
  ...
  const existing =
    await evaluationRepo.getBySample(
      payload.sample_id
    );
  ...
  // NEW FIELDS automatically forwarded
  return await evaluationRepo.create({
    ...payload,
...
/**
 * Confirms an AI evaluation and mirrors final
 * score to samples table.
 */
export async function confirmEvaluation({
  ...
  if (
    ...
    throw new Error(
      'ASV score must be between 1 and 7'
    );
  }

  const gtClass =
    mapASVToGTClass(final_asv_score);
